...
if(initial_train):
  ...
  if (apply_augmentation):
    for i in range(0,nb_initial_epochs):
        history = model.fit_generator(data_generator().flow(X_train[labeled_index], y_train[labeled_index], 
                    batch_size=32, shuffle=True),
                    steps_per_epoch=len(labeled_index), nb_epoch=1, 
                    verbose=1, callbacks=[model_checkpoint])
        
        model.save(global_path+"initmodel"+str(i)+".h5")
        log(history, i, log_file)
  else:
    history = model.fit(X_train[labeled_index], y_train[labeled_index], batch_size=32, 
              nb_epoch=nb_initial_epochs, verbose=1, shuffle=True, callbacks=[model_checkpoint])
    log(history, i, log_file)
...
for iteration in range(1, nb_iterations+1):
    ...
    X_labeled_train, y_labeled_train, labeled_index, unlabeled_index = compute_train_sets(X_train, y_train, 
              labeled_index, unlabeled_index, weights, iteration)
    ...
    model.save(global_path+"models/model"+str(iteration)+".h5")

...
f.write("Log format: iteration / epoch / nb_train / loss_train / acc_train / loss_test / acc_test\n"
        "iteration: 0 - Initialization / ~ 0 - CEAL training\n\n")

train_loop(X_labeled_train, y_labeled_train, X_test, y_test, nb_epochs, batch_size, iteration, f)
...
for iteration in range(1, nb_iterations + 1):
    ...
    en = entropy_rank(predictions)

    # labeling by Oracle process: MNIST case
    uncertain_samples = uncertain_set(en, nb_annotations)
    ...
    # (3) Training
    train_loop(X_labeled_train_aux, y_labeled_train_aux, X_test, y_test, nb_epochs, batch_size, iteration, f)
